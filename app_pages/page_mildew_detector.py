...
def page_mildew_detector_body():
    st.info(
        f"* The client is interested in telling whether a cherry leaf "
        f"contains a powdery mildew or not."
    )
    ...
    st.write(
        f"Upload cherry leaves. You may select more than one."
    )
    ...
    if images_buffer is not None:
        ...
        for image in images_buffer:
            ...
            df_report = df_report.append({"Name": image.name, 'Result':
                                          pred_class}, ignore_index=True)
